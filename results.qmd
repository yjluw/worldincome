...
4. Some countries which is not developed has encountered a problem of increasing income inequality when they were trying to develop their economic. 

In Africa, Gini of countries like `Malawi` and Mozambique all increased after some period. This phenomena is associated with their economic growth and introduction of foreign investment.

In Asia, `China` has done well in reducing income inequality from `1960-1980` but the situations worsened after 1980 which match's the period when `China` decided to develop its economic according to some newly published policy. 

In Europe, Gini of `Albania` has increased during `Albania`'s economic transition which means the transition from a planned to a market economy had a negative impact on income equality.
