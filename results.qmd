...
## **Data Collection Trends on Global Income Inequality**
...
2.  **Bottom Panel - Data Quality** reveals the composition and reliability of measurements over time, with proportions of high, average, and low-quality data for each resource category, as noted by the collector.
...
## **How has income inequality (Gini index) been distributed and changing around the world?**
...
# Create custom theme with larger text sizes
custom_theme <- theme_minimal() +
  theme(
    panel.spacing.x = unit(1, "cm"),
    # Increase title size
    plot.title = element_text(size = 20, face = "bold"),
    # Increase axis text and title sizes
...
high_ineq_plot <- ggplot(
  ...
  geom_text_repel(aes(label = country), size = 5) +  # Increased from 4 to 5
  facet_wrap(~decade) +
  scale_x_continuous(
    limits = x_limits,
...
## **Do major economic events have an effect on the income distribution of the country?**
...
## **What potential factors could indicate a country's income inequality?**
...
vcd::mosaic(gini_type ~ gdp_growth_speed + developed, 
            data = data_GDP_2020, 
            labeling_args = list(
              gp_labels = gpar(fontsize = 9) # Set font size for labels
            ),
            highlighting_fill = c("lightgreen", "lightgoldenrod", "lightcoral"),
            main = "Gini Classify by GDP and Development")
```
...
## **The Transition of Countries Across Gini Groups Over Time**
...
```{r,include=FALSE}
# Using the same data preparation as before
data_for_json <- data_GDP |>
  # Keep only necessary columns
  select(year, country, gini) |>
  # Order for cleaner data
  arrange(year, country)

# select data from year 1992 to 2021
data_for_json <- data_for_json |>
  filter(year >= 1992 & year <= 2021)
# Convert to JSON
library(jsonlite)
json_data <- toJSON(data_for_json, pretty = TRUE)

# Write to file
write(json_data, "gini_data.json")
```
...
## **How Gini of Top and bottom 5 Countries by GDP in Each Region evlovs through time**
...
3.  Europe and Oceania have lower and stable Gini coefficients, indicating more advanced social and economic policies. Economic development and inequality
