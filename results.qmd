...
```{r}
# 假设数据包含以下列：country, year, gini, gdp
# 过滤特定国家（以美国为例）
data_us <- data_median |>
...
data_us <- data_us |>
  ...
  mutate(
    gdp_growth = ifelse(
      year_diff == 1,  # 只有年份差为1时计算增长率
      (gdp - lag(gdp)) / lag(gdp) * 100,
      NA  # 否则设为NA
    )
  ) |>
  select(-year_diff)  # 移除辅助列

# 将数据从宽格式转换为长格式，以便绘制多条线
data_long <- data_us |>
  pivot_longer(cols = c(gini, gdp_growth), names_to = "indicator", values_to = "value")

# 绘制折线图
ggplot(data_long, aes(x = year, y = value, color = indicator, group = indicator)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(
  ...
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 10), # x 轴刻度字体大小
    axis.text.y = element_text(size = 10), # y 轴刻度字体大小
    axis.title = element_text(size = 12),  # 坐标轴标题字体大小
    plot.title = element_text(size = 14, face = "bold"), # 主标题字体大小
    legend.title = element_text(size = 12), # 图例标题字体大小
    legend.text = element_text(size = 10)   # 图例内容字体大小
  )
