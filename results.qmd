...
data_period <- data_GDP |>
  mutate(period = case_when(
    year >= 1950 & year <= 1975 ~ "1950-1975",
    year > 1975 & year <= 2000 ~ "1975-2000",
...
alluvial_data <- data_avg_gini |>
  select(country, period, gini_type) |> pivot_wider(names_from = period,values_from = gini_type) |>
    mutate(
    `1950-1975` = replace_na(`1950-1975`, "Missing"),
    `1975-2000` = replace_na(`1975-2000`, "Missing"),
    `2000-2022` = replace_na(`2000-2022`, "Missing")
  ) |>
  group_by(`1950-1975`,`1975-2000`,`2000-2022`) |> summarise(Freq=n())

alluvial_data <- alluvial_data |>
  mutate(
    `1950-1975` = factor(`1950-1975`, levels = c("High", "Mid", "Low", "Missing")),
    `1975-2000` = factor(`1975-2000`, levels = c("High", "Mid", "Low", "Missing")),
    `2000-2022` = factor(`2000-2022`, levels = c("High", "Mid", "Low", "Missing"))
  )


ggplot(alluvial_data,aes(axis1=`1950-1975`,axis2=`1975-2000`,axis3=`2000-2022`,y=Freq)) + 
  geom_flow(aes(fill = `2000-2022`)) +
  geom_stratum() + 
  geom_text(stat="stratum", aes(label=paste(after_stat(stratum), 
                                           "\n", after_stat(count))),size = 2) +
  scale_x_discrete(limits = c("1950-1975", "1975-2000", "2000-2022"), 
                   expand = c(0.15, 0.15)) +
  theme(plot.title = element_text(hjust=0.5))+  
  scale_fill_manual(
    values = c("Missing"="#d9d9d9","Low" = "#fecc5c", "Mid" = "#fd8d3c", "High" = "#e31a1c"),  # 顺序颜色
    name = "Gini Type"  # 图例标题
  )+
    labs(
...
```{r, fig.width=10, fig.height=14, warning=FALSE}
n <- 5  # 选择每个区域 GDP 最低的国家数量

# Step 1: 筛选每个国家至少有 5 个数据点
filtered_by_points <- data_median |>
  group_by(region_un, country) |>
  filter(n() >= 5) |>  # 确保每个国家至少有 5 个点
  ungroup()

# Step 2: 按最小 GDP 筛选每个区域的前 n 个国家
...
# Step 3: 筛选出前 n 个国家的数据
filtered_data <- filtered_by_points |>
  filter(country %in% bottom_countries_by_region)

# Step 4: 获取起始年份的数据点（用于标注国家名称）
start_points <- filtered_data |>
  group_by(region_un, country) |>
  filter(year == min(year)) |>
  ungroup()
